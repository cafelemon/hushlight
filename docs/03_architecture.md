...
> 文档版本：V0.6
> 更新日期：2026-08-20
> 状态：K0 架构评审 `approved-with-actions`
...
| 云端会话 | 会话上下文、可切换的模型/语音 Provider 路由、Policy、确认状态和成本计量 | 不假设本地动作成功；只消费 Bridge 返回的真实结果 |
...
Hushlight 的情感核心不只依赖厂商原版模型和系统提示词。项目建设自有 Gold Set、情感交互数据、偏好数据、评测协议和可迁移的 SFT/DPO 训练流程；采用 4B 级主力与 9B 级/强模型 Fallback 分层路线。V0 用户会话经可切换的云端模型/语音 Provider 提供，本机 MLX 只用于评测、训练候选和离线实验；具体基座、训练平台和供应商仍由 O-006 决策。

```mermaid
flowchart LR
    A[云端 ASR + 声学弱信号] --> C[可切换 Companion Provider]
    M[确认记忆检索] --> C
...
- Provider 适配层记录模型/语音版本、请求计量和单位成本；故障时停止本地动作候选并切换到已验证的降级回复或可用 Provider，不能默认为成功或重放 L3 请求。
...
| 本地实现 | 原生 Swift 6/SwiftUI、系统接口、安装和菜单栏 | 原生 .NET、Windows 系统接口、安装和托盘 | 不要求使用同一桌面框架 |
| V0 完成度 | 网易云、微信、系统音量、提醒、打开内容形成真实闭环 | 安装、绑定、托盘、权限、暂停、诊断和 `bridge-v1` 契约验证 | 工具名、参数、权限和结果 Schema 一致 |
| 测试 | 真实账号/真实应用支持版本矩阵、失败路径与结果回传 | 全新环境安装、权限拒绝/恢复、暂停、诊断与契约测试 | 消息安全 Gate 与公共语义一致 |
...
- Windows .NET 的安装、权限、托盘、暂停、诊断实现与 `bridge-v1` 契约测试；macOS 版本矩阵仍需 S1 实机冻结。
...
- 主力/Teacher/Fallback 模型与版本、训练平台、云端 Provider、成本路由和降级策略。
- `companion-state-v1` 枚举、置信度校准和 API Schema。
- 具体 ASR/TTS Provider 与故障切换阈值。
- 消费版外观、在场传感器和量产硬件方案。
- 自研板具体摄像头、电机/驱动/限位、原理图、BOM、首板排期和独立预算审批。
