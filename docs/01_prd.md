...
> 文档版本：V0.3
> 更新日期：2026-08-20
> 状态：V0.2 PRD 已评审通过；V0.3 K0 基线变更已记录
...
9. 记忆可见、可改、可删。
10. 不以依赖和对话时长作为增长目标。
11. 采集状态必须看得见、关得掉，关键隐私边界不依赖用户相信软件提示。
...
| FR-DEV-004 | 摄像头只在端侧输出对话者位置，不上传或保存原始图像；提供硬件联动工作指示、物理遮挡和相机断电 | P0 | 遮挡后相机供电关闭、无有效帧，用户能直接判断状态 |
| FR-DEV-007 | 提供锁定式物理静音：硬断独立麦克风供电、独立红灯、断线默认静音；解除后不自动采集 | P0 | MCU 未运行时仍切断麦克风供电并亮红灯；解除后需再次主动唤醒 |
...
| FR-BRG-010 | V0 并行提供 macOS 与 Windows Bridge 基础产品化能力 | P0 | macOS 完成完整本地适配闭环；Windows 完成安装、绑定、托盘、权限、暂停、诊断和共享契约验证 |

V0 的用户完整闭环以 macOS 为准：网易云音乐、微信草稿/确认发送、云端会话与真实结果回传均须完成。Windows 不复制 macOS 技术栈，使用原生 .NET；完整网易云/微信本地适配进入 V1 Alpha，但两端必须遵守同一 `bridge-v1` 工具、权限、确认、错误和结果契约。

V0 用户会话通过可切换的云端模型/语音 Provider 提供；本机 MLX 仅用于评测、训练候选和离线实验，不能作为用户可用推理、延迟或成本承诺。
...
V0 的首个音乐适配对象为网易云音乐，首个聊天适配对象为微信。macOS 必须完成两者的真实闭环；Windows 在 V0 仅完成共享契约和基础产品化能力。两端可以采用不同本地实现，但不得改变用户可见流程、工具语义和安全规则。
...
- 记忆纠正率及纠正后复发率。
- 单活跃用户月云成本。
- 免费种子测试中的付费意愿和价格接受度；订阅、续费和取消率属于 V1 以后验证。
...
- 10 至 20 名成年种子用户在免费测试中完成核心体验；不收款、不交付订阅。
- 非技术用户无需命令行完成 Bridge 安装、绑定和首个动作。
- 用户能自然完成聊天、音乐、提醒和消息草稿流程。
- macOS 上至少一个音乐软件和一个聊天软件适配稳定通过验收；Windows 仅完成其限定的基础产品化与契约验收。
- 所有安全护栏指标满足要求。
- Bridge 离线和工具失败均能正确降级。
- 得到真实云成本和适配维护成本数据。
- 用户研究能够回答“硬件存在感是否优于只用聊天应用”。

H0C Rev A 仅用于内部联调。只有完成 H-IO/H-CAM/H-MOT/H-ENC/H-MIC/H-FFC、ERC、独立预算审批和 3 块同版本稳定工程板切换 Gate 后，才可成为主要开发设备；它不是种子用户交付物，也不构成量产承诺。
