...
| 模拟麦克风 | 2 × Infineon `IM73A135V01` 同批次模拟差分 MEMS | `FROZEN-A` | 73dB(A) SNR、135dBSPL AOP、模拟差分输出；两只必须同批次、同声学结构，接既定 ES7210 |
...
采用 `3PDT` 锁定式滑动开关，三组触点必须独立实现：

1. 直接切断两颗 IM73A135V01 的独立 `MIC_2V8` 供电；
2. 直接点亮红色 `MUTE` LED，不经 MCU；
3. 生成 `MUTE_SENSE` 给 BASE-S3；断线、松脱或 MCU 未运行时按硬件/软件默认静音处理。

红灯不能完全依赖固件。解除静音后硬件恢复供电，但固件保持 `Idle`，不自动开始上传。Head 侧另设手动机械镜头遮挡片，关闭时须以联动双触点开关切断相机供电、输出 `SHUTTER_CLOSED_N`；绿色 `CAM_ACTIVE` LED 由相机实际供电后的硬件路径驱动。打开快门后不自动进入视觉跟随。
