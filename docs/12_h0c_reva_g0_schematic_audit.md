...
> 阶段留存提示（2026-08-20）：本文保留 126–212 与 209–212 的历史审计语境，不再作为在线施工指令。当前施工以 [在线工程逐页接线清单](wiring/README.md) 为准；`01POWERUSB` 当前有效编号为 1–208。
>
> 日期：2026-08-17
> 状态：P0-01…P0-06、01-D2-01 与 06 页器件级 P1-02 已关闭；01 页新增外围已落位并发布 126–212 接线表。仍不代表接线完成、ERC、PCB、BOM、打板或采购放行
> 范围：审计、器件核对、EDA 定向修复与文档收敛；未运行 ERC、未转 PCB

## 1. 结论

六项 01 页 P0 已在设计与对象层面闭环：CC 直连 STUSB4500、01 不重复 I²C 上拉、U6.EN 撤销高压直连，`F1/F2=0/0`，入口保护改为 `U17=TPS259474LRPWR`，并按实际位号发布 USB/STUSB4500/主输入 eFuse 的 126–212 单点表。2026-08-17 对 01 A 表全部引用位号做对象搜索，唯一缺件 `D2` 已按冻结选型补放为 `TPD2EUSB30ADRTR/C94934`，保存后为 `D2=1/1`。旧 19–22 的 U6 输入标签必须按 209–212 原位改为 `VBUS_BUCK_IN`，否则会旁路 U17。06 页 `P1-02` 保持 Closed；01 接线、ERC 与台架 Gate 仍未完成。
...
| `01-D2-01` | Closed | 126–212 引用了 USB2 数据 ESD `D2`，但对象搜索曾为 `0/0` | 11 第 3.5.1 节与 EDA 对象/属性复核 | 已补放并保存 `D2=TPD2EUSB30ADRTR/C94934`；对象搜索为 `1/1`，136/140/142 现可执行。此件只保护 D+/D−，不得与已删除的 CC 保护候选 D4 混淆 |
